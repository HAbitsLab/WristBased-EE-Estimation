...
def generate_table(PATH_RESAMPLE_ACC, PATH_RESAMPLE_GYRO, ROOT_PATH_FSM, met_cart_dic, participant_weight, activity_estimate, p):
        ...
        try:
            if(df_logs['Include'][i]==1):
                ...
                for j in range(int((et - st).seconds / 60)):
                    if(j <= 1): # exclude first two minutes
                        ...
                        data_training.append(get_train_data(df_gyro, start_time, 'gyro') + get_train_data(df_acc, start_time, 'acc'))           
                        start_time += pd.DateOffset(minutes=1)
            else:
    ...
    for i in range(len(data_training)):
        for j in range(6):
            if(data_training[i][j].shape[0] != 1200):
                s_temp = pd.Series([0]*(1200-data_training[i][j].shape[0]))
